...
ORDER_TRANSITIONS: dict[str, frozenset[str]] = {
    ...
    "PAID": frozenset({"FULFILLING", "REFUNDED"}),
...
async def get_or_create_cart(session: AsyncSession, tenant_id: UUID, discord_user_id: int) -> Cart:
    result = await session.execute(
        select(Cart).where(Cart.tenant_id == tenant_id, Cart.discord_user_id == discord_user_id)
    )
...
async def create_order_from_cart(
    ...
    result = await session.execute(
        ...
        .where(CartItem.cart_id == cart.id, Product.tenant_id == tenant_id, Product.active.is_(True))
    ...
    # Rows are locked inside the transaction. The stock decrement is conditional,
    # so concurrent checkouts cannot drive stock below zero.
    order = Order(
        id=uuid4(),
        tenant_id=tenant_id,
