    ...
    try:
        ...
        if payment.reconcile_attempts >= MAX_OUTBOX_ATTEMPTS:
            ...
            enqueue_outbox(
                ...
                aggregate_type="payment",
                aggregate_id=str(payment.id),
                event_type="payment.reconciliation_dead_lettered",
                payload={"order_id": str(payment.order_id), "error": str(exc)[:2000]},
            )
            return "dead_lettered"
        payment.next_reconcile_at = datetime.now(timezone.utc) + timedelta(
            seconds=exponential_backoff(payment.reconcile_attempts, RECONCILE_BASE_DELAY_SECONDS, RECONCILE_MAX_DELAY_SECONDS)
        )
...
class OutboxDispatcher:
    ...
    async def dispatch(self, session: AsyncSession, event: OutboxEvent) -> None:
        ...
        # Events without a local side-effect handler are still acknowledged.
        # Future Discord delivery/email/analytics consumers can be registered
        # without changing the transactional producer.
