...
async def _requeue_stale_outbox(session) -> None:
    ...
    result = await session.execute(
        select(OutboxEvent)
        .where(OutboxEvent.status == "PROCESSING", OutboxEvent.locked_at < cutoff)
                ...
                async with session.begin():
                    ...
                    if event.attempts >= MAX_OUTBOX_ATTEMPTS:
                        ...
                        if event.event_type == "refund.execute":
                            ...
                            if refund_id:
                                refund = await session.scalar(
                                    select(RefundRecord)
                                    .where(RefundRecord.id == UUID(str(refund_id)))
                                    .with_for_update()
                                )
                                if refund is not None and refund.status not in {"APPROVED", "CANCELLED"}:
                                    ...
                                    order = await session.scalar(
                                        select(Order)
                                        .where(Order.id == refund.order_id, Order.tenant_id == refund.tenant_id)
                                        .with_for_update()
                                    )
                        ...
                        event.next_attempt_at = datetime.now(timezone.utc) + timedelta(
                            seconds=exponential_backoff(
                                event.attempts, OUTBOX_BASE_DELAY_SECONDS, OUTBOX_MAX_DELAY_SECONDS
                            )
                        )
    return processed
...
async def execute_fulfillment(session, event: OutboxEvent, role_delivery: DiscordRoleDelivery) -> None:
    ...
    if record.delivery_type == "digital_link":
        if not record.delivery_url:
            raise RuntimeError("digital_delivery_url_missing")
        await mark_fulfillment_delivered(session, record.id)
        return
    if record.delivery_type == "discord_role":
        if record.discord_guild_id is None or record.discord_role_id is None:
            raise RuntimeError("discord_role_configuration_missing")
        await role_delivery.add_role(record.discord_guild_id, record.discord_user_id, record.discord_role_id)
        await mark_fulfillment_delivered(session, record.id)
        return
...
async def execute_revoke(session, event: OutboxEvent, role_delivery: DiscordRoleDelivery) -> None:
    ...
    if record.delivery_type == "discord_role" and record.discord_guild_id and record.discord_role_id:
        await role_delivery.remove_role(record.discord_guild_id, record.discord_user_id, record.discord_role_id)
    await revoke_fulfillment(session, fulfillment_id)


async def main() -> None:
    provider = MercadoPagoPixProvider()
    role_delivery = DiscordRoleDelivery()
    ...
    async def revoke_handler(session, event: OutboxEvent) -> None:
        await execute_revoke(session, event, role_delivery)
    ...
    dispatcher.register("fulfillment.revoke", revoke_handler)
