...
aasync function api<T>(path: string): Promise<T> {
      ...
      <section className="content">
        <header className="topbar">
          <div>
            ...
            <h1>{tenant?.name || 'Selecione uma loja'}</h1>
          </div>
          <div className="toolbar">
            <select value={tenantId} onChange={(event) => setTenantId(event.target.value)}>
              {me.tenants.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.role}</option>)}
            </select>
        ...
        <section className="grid">
          ...
          <article className="card"><span>Pedidos</span><strong>{overview?.orders ?? 0}</strong><small>Últimos registros</small></article>
            ...
            <table>
              ...
              <tbody>
                {orders.slice(0, 12).map((order) => (
                  <tr key={order.id}><td>#{order.id.slice(0, 8)}</td><td>{order.discord_user_id}</td><td><span className={`status ${order.status.toLowerCase()}`}>{order.status}</span></td><td>{money(order.total_minor, order.currency)}</td><td>{new Date(order.created_at).toLocaleString('pt-BR')}</td></tr>
                ))}
